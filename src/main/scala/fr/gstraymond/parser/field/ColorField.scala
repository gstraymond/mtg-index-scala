...
trait ColorField {
  ...
  def _colors(maybeCastingCost: Option[String],
              hints: Seq[String],
              additionalColors: Option[Seq[String]]): Seq[String] = {
  ...
  def _dualColors(maybeCastingCost: Option[String],
                  additionalColors: Option[Seq[String]]): Seq[String] = {
    ccWithHint(maybeCastingCost, additionalColors, Nil) match {
      ...
      }.map { case Seq(c1, c2) => s"${c1.lbl} OR ${c2.lbl}" }
      case _ => Nil
    }
  }

  def _tripleColors(maybeCastingCost: Option[String],
                    additionalColors: Option[Seq[String]]): Seq[String] = {
    ccWithHint(maybeCastingCost, additionalColors, Nil) match {
      case (castingCost, false) if castingCost != "" => {
        ALL_TRIPLE_COLORS.filter(find(castingCost)(_).size == 3) match {
          case seq if seq.size == 1 => seq
          case seq if seq.size > 1 => Nil
          case _ => ALL_TRIPLE_COLORS.filter(find(castingCost)(_).size == 2) match {
            case seq if seq.size == 3 => seq
            case _ => ALL_TRIPLE_COLORS.filter(find(castingCost)(_).size == 1)
          }
        }
      }.map { case Seq(c1, c2, c3) => s"${c1.lbl} OR ${c2.lbl} OR ${c3.lbl}" }
      case _ => Nil
    }
  }

  private def ccWithHint(maybeCastingCost: Option[String],
