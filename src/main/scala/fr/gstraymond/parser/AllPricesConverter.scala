...
case class CardPrice(uuid: String,
                    paper: Option[Price],
                    online: Option[Price])

case class Price(normal: Option[Double],
                foil: Option[Double])

object PriceFormats {
  implicit val cardPriceFormat: JsonValueCodec[Seq[CardPrice]] = JsonCodecMaker.make(CodecMakerConfig.withTransientEmpty(false))
}