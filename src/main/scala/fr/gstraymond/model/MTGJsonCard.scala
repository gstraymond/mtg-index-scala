...
case class MTGJsonPrice(paper: Option[MTGJsonPricePaper],
                        mtgo: Option[MTGJsonPriceOnline])

case class MTGJsonPricePaper(cardkingdom: Option[MTGJsonPriceProvider],
                            tcgplayer: Option[MTGJsonPriceProvider])
...
object MTGJsonFormats {
  implicit val mtgJsonEditionFormat: JsonValueCodec[MTGJsonAllPrintings] = JsonCodecMaker.make(CodecMakerConfig.withTransientEmpty(false))
  implicit val mtgJsonAllPricesFormat: JsonValueCodec[MTGJsonAllPrices] = JsonCodecMaker.make(CodecMakerConfig.withTransientEmpty(false).withMapMaxInsertNumber(100000))
}
