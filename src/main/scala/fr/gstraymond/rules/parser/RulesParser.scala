...
import fr.gstraymond.rules.model.{Rule, RuleLink, Rules}
...
object RulesParser extends Log {
  ...
  def parse(filename: String,
    ...
    val emptyMap = ((lines.head -> false) +: lines.tail.zip(lines).map { case (line, prev) =>
      line -> prev.isEmpty
    ...
    val rules = nonEmptyLines.zipWithIndex.map {
      case (line, index) if line.head.isDigit && !glossaryRange.contains(index) =>
        ...
        if (index <= creditsMenuIndex) Rule(id = None, text = split(1), links = Seq(RuleLink(id, 0, split(1).length - 1)), level + 1)
    ...
    val rules1 = rules.zipWithIndex.map { case (rule, i) =>
      if (range.contains(i) && rule.text.exists(_.isDigit)) {
        ids.filter(r => rule.text.contains(r.pattern.toString)).foldLeft(rule) { (acc, id) =>
          val links = id.findAllMatchIn(rule.text).map { r =>
            RuleLink(id.pattern.toString, r.start, r.end - 1)
          }.filterNot { link =>
            val linkRange = link.start to link.end
            acc.links.exists { accLink =>
              (accLink.start to accLink.end).containsSlice(linkRange)
            }
          }
